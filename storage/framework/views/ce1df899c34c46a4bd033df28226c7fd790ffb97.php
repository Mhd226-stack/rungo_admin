  ...
  cursor:pointer;
    ...
    <script>
      ...
      function update_notification_count(chat_id = null)
      {
        if(chat_id != null)
          {
            var data = {chat_id:chat_id,user_id:'<?php echo e(Auth::user()->id); ?>'};
            $.ajax({
...
<div class="messaging">
     ...
     <!--      <div class="headind_srch">
          ...
          <div class="inbox_chat">
            ...
            <?php $__currentLoopData = $user_details; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key=>$value): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
          ...
          <?php
            ...
            ?>
            <div class="chat_list active_chat" data-val="<?php echo e($value->id); ?>">
            <?php else: ?>
            <div class="chat_list" data-val="<?php echo e($value->id); ?>">
            <?php endif; ?>

              <div class="chat_people chat-user-info-img">
                <div class="chat_img"> <img src="<?php echo e($value->user_detail->profile_picture); ?>" style="width: 50px; aspect-ratio: 1; border-radius: 50%;"> </div>
                <div class="chat_ib">

                  <h5><?php echo e($value->user_detail->name); ?>

                    <?php if($user_datas->slug =="user"): ?>
                    <span class="label label-success" style="float: none; margin-left: 8px; /* margin-top: -15px; */">User</span>
                    <?php endif; ?>
                    <?php if($user_datas->slug =="owner"): ?>
                   <span class="label label-green" style="float: none;margin-left: 8px;/* margin-top: -15px; */background-color: green;">owner</span>
                    <?php endif; ?>
                    <?php if($user_datas->slug =="driver"): ?>
                    ...
                    <?php endif; ?>
                    <span class="chat_date"> <?php echo e($time); ?> </span></h5>
                  <p><?php echo e($value->message); ?>

                  <?php
            $unseen_count = DB::table('chat_messages')->where('chat_id',$value->id)->where('to_id',Auth::user()->id)->where('unseen_count',0)->count();
                  ?>
                  <?php if($unseen_count > 0 && $key != 0): ?>

                  <span class="notication-count <?php echo e($value->id); ?>" style=" float: right; background-color: red; padding: 4px;  font-size: 9px; color: white; font-weight: bold;
                  border-radius: 100%;  position: relative; top: -2px;"><?php echo e(Auth::user()->id); ?></span>
                  <?php endif; ?>
                  </p>
                </div>
              </div>
      ...
      <script>
        ...
        chatmessage_get('<?php echo e($chat_id); ?>');
      </script>
      </div>
...
function displayMessages(messageData)
{
    ...
    if(messageData.chat_id == $(".chat_list.active_chat").attr("data-val") && user_id != messageData.from_id)
    {
      if(messageData.message !== null && messageData.message !== "" && messageData.message !== undefined)
      {
          ...
          $(".incoming_msg_data").append('<div class="incoming_msg" id="last_message"> <div class="incoming_msg_img"> <img src="<?php echo e(Auth::user()->profile_picture); ?>"> </div><div class="received_msg">  <div class="received_withd_msg"><p>'+messageData.message+'</p><span class="time_date"> Just Now</span></div> </div> </div>').promise().done(function(){
          var lastMessage = document.getElementById('last_message');
          // Scroll the last message into view
          lastMessage.scrollIntoView({ behavior: 'smooth', block: 'end' });
          });
...
<?php
     ...
     var chat_count = '<?php echo e(count($chat_ids)); ?>';
...
database.ref('chats/<?php echo e($v); ?>').on('value', (snapshot) => {

            $i = '<?php echo e($k); ?>';
            $count = '<?php echo e(count($chat_ids)-1); ?>';
          if(!initialLoad_dt)
          {
...
<?php echo $__env->make('admin.layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\laragon\www\taxi\resources\views/admin/master/chat.blade.php ENDPATH**/ ?>